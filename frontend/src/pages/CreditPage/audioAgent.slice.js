    ...
    try {
      const response = await axios.post(
        BASE_URL +
          ...
          `thread_id=${payload?.threadId}&state=${payload?.state}&translate=false&document_upload_flag=${payload?.uploadFlag}&offer_item_id=${payload?.offer_item_id}&selected_loan_amount=${payload?.selected_loan_amount}&language=${payload?.language}&agent_gender=${payload?.agentGender}`,
        payload?.file,
        {
          headers: {
...
export const startConversion = createAsyncThunk(
  ...
  async (payload) => {
    const response = await apiRequest(
      "POST",
      `${apis?.startConversionApi}?language=${payload?.language}&agent_gender=${payload?.agentGender}`
    );
    return response;
  }
);
...
export const bankLoanDataResumeConversion = createAsyncThunk(
  ...
  async (payload) => {
    const response = await apiRequest(
      "POST",
      `${apis?.bankLoanDataApi}?language=${payload?.language}&agent_gender=${payload?.agentGender}`,
      payload
    );
    return response;
  }
