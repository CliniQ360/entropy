...
async def start_conversation():
    try:
        ...
        return ConversationController().start_conversation(thread_id)
...
async def upload_documents(files: Optional[List[UploadFile]] = File(None)):
    try:
        ...
        return ConversationController().upload_documents(files=files)
    except Exception as error:
...
def get_state(thread_id: str):
    try:
        ...
        return ConversationController().get_state(thread_id=thread_id)
    except Exception as error:
...
def resume_conversation(request: ConversationResume):
    try:
        ...
        return ConversationController().resume_conversation(**request_dict)
    except Exception as error:
